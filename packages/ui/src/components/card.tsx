...
function Card({
	...
	return (
		<div
			...
			data-size={size}
			className={cn(
				"group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl bg-card py-(--card-spacing) text-card-foreground text-sm/relaxed ring-1 ring-border [--card-spacing:--spacing(5)] has-[>img:first-child]:pt-0 has-data-[slot=card-footer]:pb-0 data-[size=sm]:has-data-[slot=card-footer]:pb-0 data-[size=sm]:[--card-spacing:--spacing(4)]",
				className,
			)}
			{...props}
		/>
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn(
				"cn-font-heading font-medium text-base tracking-tight group-data-[size=sm]/card:text-sm",
				className,
			)}
			{...props}
		/>
